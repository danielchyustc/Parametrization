...
double CdVARAP::Weight(Matrix3d xt, int i, int j)
{
	int k = 0;
	while (k == i || k == j)
		k++;
...
double CdVARAP::Weight(Matrix3d xt, int i)
{
	int j = 0, k = 0;
	while (j == i)
		j++;
...
double CdVARAP::Energy()
{
	...
	double energy = pow((para_area_ratio - mesh_area_ratio).norm(), 2);
	for (size_t i = 0; i < nV; i++)
	{
		auto vi = heMesh->Vertices().at(i);
		for (auto vj : vi->AdjVertices())
		{
			...
			energy += CotanWeight(vi, vj) * pow((para_map_.col(i) - para_map_.col(j)).norm(), 2);
		}
	}
}